...
var pgToTsType = (pgType, isNullable) => {
  ...
  const tsType = types2[pgType] || "unknown";
  return isNullable ? `${tsType} | null` : tsType;
};
function generateTableTypes(tableName, sql) {
  return __async(this, null, function* () {
...
function generateTypes(options) {
  return __async(this, null, function* () {
    ...
    const sql = src_default(options.url);
    try {
      (0, import_fs2.mkdirSync)(options.output, { recursive: true });
      if (options.table) {
        process.stdout.write(`Generating types for ${options.table} `);
        const dots = setInterval(() => process.stdout.write("."), 500);
        const tableType = yield generateTableTypes(options.table, sql);
        clearInterval(dots);
        process.stdout.write("\n");
        const outputPath = import_path.default.join(options.output, `${options.table}.ts`);
        (0, import_fs2.writeFileSync)(outputPath, tableType);
     ...
     `);
        ...
        for (const row of tables) {
          ...
          const tableType = yield generateTableTypes(row.table_name, sql);
        ...
        (0, import_fs2.writeFileSync)(outputPath, typesContent);
